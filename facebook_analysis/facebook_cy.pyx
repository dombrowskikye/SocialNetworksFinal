...
def read_facebook_network_cy(str filename):
    cdef:
        list edges = []
        int user1, user2
    
    with open(filename, 'r') as f:
        for line in tqdm(f, desc="Loading Facebook network"):
            if line.strip():
                try:
                    # Facebook edges are undirected, so we only need one entry
                    user1, user2 = map(int, line.strip().split())
                    edges.append((user1, user2))
                except ValueError:
                    continue
    
    # Create undirected graph (Facebook friendships are mutual)
...
def calculate_betweenness_sample_cy(G, int k, bint normalized=True, int seed=-1):
    ...
    # For Facebook, we might want to adjust sampling based on degree
    degrees = dict(G.degree())
    probabilities = [degrees[node]/sum(degrees.values()) for node in nodes]
    pivots = list(np.random.choice(nodes, min(k, n), p=probabilities, replace=False))
...
def efficient_layout_cy(G, float k=-1.0, int iterations=50, int seed=42):
    ...
    for i in range(iterations):
        ...
        # Calculate forces - optimized for undirected graph
        for node1 in G:
...
def create_output_directory():
    ...
    output_dir = Path(f"facebook_analysis_cy_{timestamp}")
    output_dir.mkdir(exist_ok=True)
    return output_dir

def plot_network_visualization_cy(G, dict bt_centrality, list communities, output_dir):
    cdef int TOP_NODES = 1000  # Adjust based on Facebook network density
    ...
    try:
        pos = efficient_layout_cy(G_sub, iterations=50)
    ...
    plt.title(f'Facebook Network Visualization (Top {TOP_NODES} nodes)\nNode size: Betweenness Centrality, Color: Community')
    plt.axis('off')
    plt.savefig(output_dir / 'network_visualization.png', dpi=150, bbox_inches='tight')
    plt.close()

def analyze_facebook_network_cy(str filename):
    cdef:
        float start_time = time.time()
        dict bt_centrality
        list communities
    
    print("Starting Cython-optimized Facebook network analysis...")
    output_dir = create_output_directory()
    
    G = read_facebook_network_cy(filename)
    print(f"Network loaded in {time.time() - start_time:.2f} seconds")
    
    stats = {
        "nodes": G.number_of_nodes(),
        "edges": G.number_of_edges(),
        "components": nx.number_connected_components(G),  # Use connected instead of weakly_connected for undirected
        "density": nx.density(G),
        "average_clustering": nx.average_clustering(G)  # Add clustering for Facebook
    }
    print("\nNetwork Statistics:")
    ...
    # Save additional Facebook-specific statistics
